...
//   argvOn  -> used instead of `argv` when the tool has no `toggle` verb, and
//   argvOff    otherwise, so a flip is idempotent rather than trusting that
//              the last state read is still current.

function commands() {
  return [
    // ---------------------------------------------------------- spotlight
    { key: "spotlight.settings", title: "Edit Spotlight Settings", subtitle: "Create defaults if needed, then open the file", icon: "󰒓", kind: "spotlight-settings", keywords: "spotlight settings preferences config" },
    ...
    { key: "spotlight.reset",    title: "Reset Spotlight Learning", subtitle: "Delete only Spotlight's local usage file", icon: "󰑐", kind: "spotlight-reset", keywords: "spotlight settings reset clear forget learning ranking history", confirm: true },
    ...
    { key: "spotlight.shortcut", title: "Change Spotlight Shortcut", subtitle: "Set, change or restore the key that opens Spotlight", icon: "󰌌", kind: "spotlight-shortcut", keywords: "spotlight shortcut keybinding hotkey bind key rebind restore" },
    ...
    // win.trans, win.layout and win.tfs get no `state` on purpose. Transparency
    // and tiled fullscreen belong to the focused window and layout belongs to
    // the active workspace and is dwindle/scrolling rather than on/off, so none
    // of the three has a global state a switch could honestly show.
    ...
    // nmcli has no `toggle` verb, so this is the one entry that needs the
    // explicit on/off pair; the row already knows which way to go.
    { key: "dev.wifi",    title: "Toggle Wi-Fi",             subtitle: "Radio on or off",             icon: "󰖩", kind: "shell", argvOn: ["nmcli", "radio", "wifi", "on"], argvOff: ["nmcli", "radio", "wifi", "off"], state: "wifi", keywords: "wifi wlan wireless network radio internet" },
