...
// Applications are scored by the shell's own AppLibrary so they keep ranking
// identically to the Omarchy menu. Files use their own scorer, FileRank.js:
// this one matches a composed haystack (title + subtitle + keywords +
// accessory) and rejects outright with -1, neither of which fits a file -
// the match has to be judged on the basename alone (the directory is noise
// a deep, unrelated path could otherwise ride to the top on), and it can
// never reject, since `fd` already decided the candidate belongs.
...
if (typeof module !== "undefined") {
  module.exports = { normalize: normalize, acronym: acronym, score: score, rank: rank }
}
