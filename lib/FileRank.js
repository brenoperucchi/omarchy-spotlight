...
// passes through a dotdir - capped well under FILE_TIER_STEP so it can never
// carry a hit across a tier boundary, the same invariant Frecency.weight
// documents for usage nudges: match quality always wins. Computed relative
// to `searchRoot`, not an absolute segment count, so a dotdir the user
// explicitly navigated into (`~/.config/retroarch/...`) is not penalized for
// being hidden or deep - only what lies below the root they chose is.
function pathPenalty(dir, name, searchRoot) {
  var root = String(searchRoot || "").replace(/\/+$/, "")
  var d = String(dir || "")
  var rel = d.indexOf(root) === 0 ? d.slice(root.length) : d
...
function rank(list, pattern, searchRoot, maxTerms) {
  ...
  var scored = list.map(function(f) {
    ...
    return { f: f, score: tier * FILE_TIER_STEP - penalty }
  })
