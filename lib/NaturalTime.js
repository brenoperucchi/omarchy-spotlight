...
function dayAnchor(now, word) {
  ...
  if (WEEKDAYS[w] !== undefined) return nextWeekday(now, WEEKDAYS[w])
  return null
}
...
function parseWhen(text, now) {
  ...
  if (m) {
    ...
    var per = UNIT_MINUTES[m[2].toLowerCase()]
    if (isFinite(n) && per) {
  ...
  if (m) {
    var at3 = new Date(now.getTime() + UNIT_MINUTES[m[1].toLowerCase()] * 60000)
  ...
  if (m) {
    ...
    if (MONTHS[monthWord] !== undefined && dayNum >= 1 && dayNum <= 31) {
      var hourM = m[5] ? normalizeHour(Number(m[5]), (m[7] || "").toLowerCase()) : 9
      var atMon = new Date(now.getFullYear(), MONTHS[monthWord], dayNum, hourM, m[6] ? Number(m[6]) : 0, 0, 0)
...
function parseEvent(text, now) {
  ...
  if (dm) {
    var per = UNIT_MINUTES[dm[2].toLowerCase()]
  ...
  if (EVENT_NOUNS[triggerWord] && /^(with|w\/|re|about|für|fuer)\b/i.test(title))
    title = EVENT_NOUNS[triggerWord] + " " + title
