...
function findFamily(unit) {
  for (var i = 0; i < FAMILIES.length; i++) {
    if (FAMILIES[i].units[unit] !== undefined) return FAMILIES[i]
  }
  return null
}
...
function convert(src) {
  ...
  if (fromFamily.temperature) {
    out = fromFamily.units[to].from(fromFamily.units[from].to(amount))
  } else {
    out = amount * fromFamily.units[from] / fromFamily.units[to]
  }
