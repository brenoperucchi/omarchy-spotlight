...
//
// Spotlight normally asks the shell's own AppLibrary for its application
...
//
// Candidate filtering and provider-local ordering are a port of Omarchy's own
// services/AppSearch.js (MIT). Spotlight applies its shared cross-provider
// score after these candidates have been projected into rows.

var FIELD_CHARS = 512
var SEARCH_CHARS = 2048
var KEYWORD_COUNT = 64
// A Chrome-installed web app's .desktop id is a generated identifier, not
// something a user would ever intentionally type: chrome-<32-char extension
// id, base16 with a-p instead of 0-9a-f>-<Chrome profile directory name>
// ("Default", "Profile 1", ...). Found via a real bug report: three
// unrelated PWAs (installed under the "Default" Chrome profile) matched
// every search for the word "default", because that id was being folded
// into their searchable text below. None of an id shaped like this is
// human-meaningful, so it is excluded wholesale rather than trimmed.
var CHROME_WEBAPP_ID_RE = /^chrome-[a-p]{32}-.+$/i
...
function entrySearchText(entry) {
  ...
  return [
    ...
    keywordText(entry),
    CHROME_WEBAPP_ID_RE.test(id) ? "" : id
